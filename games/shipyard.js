...
import { vec3, vec4, quat, mat4, mat3, V } from "../sprig-matrix.js";
import { mergeMeshes, transformMesh, validateMesh, } from "../render/mesh.js";
import { assert, range } from "../util.js";
import { centroid, quatFromUpForward, } from "../utils-3d.js";
...
function getPathFrom2DQuadMesh(m, up) {
...
function snapXToPath(path, x, out) {
...
export function createHomeShip() {
    ...
    let longestRibIdx = 0;
    {
        ...
        for (let curve of ribCurves) {
            ...
            const even = createEvenPathFromBezier(topToBottomCurve, plankWidth * 2.0, // * 0.95,
            [1, 0, 0]);
            // even.reverse();
...
function translatePath(p, tran) {
    p.forEach((n) => vec3.add(n.pos, tran, n.pos));
    return p;
}
const __temp3 = vec3.create();
function translatePathAlongNormal(p, t) {
    p.forEach((n) => {
...
function mirrorPath(p, planeNorm) {
    // TODO(@darzu): support non-origin planes
    if (DBG_ASSERT)
        assert(Math.abs(vec3.sqrLen(planeNorm) - 1.0) < 0.01, `mirror plane must be normalized`);
    let a = planeNorm[0];
    let b = planeNorm[1];
    let c = planeNorm[2];
...
function bezierPosition(b, t, out) {
...
function bezierTangent(b, t, out) {
    const t0 = 3 * (1 - t) ** 2;
    const t1 = 6 * (1 - t) * t;
    const t2 = 3 * t ** 2;
    out[0] =
...
function createPathFromBezier(b, nodeCount, up) {
    assert(nodeCount >= 2);
    const path = [];
...
function createEvenPathFromBezier(b, spacing, up) {
    const path = [];
