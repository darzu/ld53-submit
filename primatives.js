...
import { seqEndesga16 } from "./color/palettes.js";
import { cloneMesh, mapMeshPositions, scaleMesh, scaleMesh3, transformMesh, } from "./render/mesh.js";
...
export function makePlaneMesh(x1, x2, z1, z2) {
    const res = {
...
export function resetFlatQuadMesh(width, height, mesh, doubleSided = false) {
    ...
    // create each quad
    {
        ...
        for (let x = 0; x < height - 1; x++) {
            for (let z = 0; z < width - 1; z++) {
                vec4.set(idx(x, z + 1), //
                idx(x + 1, z + 1), idx(x + 1, z), idx(x, z), mesh.quad[i]);
                i++;
                if (doubleSided) {
                    vec4.set(idx(x, z), //
                    idx(x + 1, z), idx(x + 1, z + 1), idx(x, z + 1), mesh.quad[i]);
                    i++;
                }
...
export function createFlatQuadMesh(width, height, doubleSided = false) {
    assert(width > 1 && height > 1);
...
export function mkOctogonMesh() {
    ...
    }, mat4.fromRotationTranslationScaleOrigin(quat.IDENTITY, [-1.5, 0, -1.5], [0.2, 0.2, 0.2], [1.5, 0, 1.5]));
}
export function mkHalfEdgeQuadMesh() {
    return transformMesh({
        pos: [V(0, 0, 0), V(0, 0, 3), V(3, 0, 3), V(3, 0, 0)],
        tri: [],
        quad: [vec4.clone([0, 1, 2, 3])],
        colors: [vec3.clone(V(0, 0, 0))],
    }, mat4.fromRotationTranslationScaleOrigin(quat.IDENTITY, [-1.5, 0, 0], [0.4, 0.2, 0.2], [1.5, 0, 0]));
}
...
export const SHIP_OFFSET = V(3.85 - 2.16, -0.33 - 0.13, -8.79 + 4.63);
...
export function makeDome(numLon, numLat, r) {
    ...
    for (let lat = 0; lat <= numLat; lat++) {
        ...
        for (let lon = 0; lon < numLon; lon++) {
            ...
            if (lat === 0)
                break; // at the tip-top, we only need one pos
            if (lat === 1) {
                // top triangles
                let i3 = pos.length - 2;
                if (i3 == 0)
...
export function createRudderMesh() {
