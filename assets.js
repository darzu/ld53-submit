...
import { cloneMesh, getAABBFromMesh, getMeshAsGrid, mapMeshPositions, mergeMeshes, normalizeMesh, scaleMesh, scaleMesh3, transformMesh, validateMesh, } from "./render/mesh.js";
...
import { computeTriangleNormal, farthestPointInDir, normalizeVec2s, } from "./utils-3d.js";
import { onInit } from "./init.js";
import { VERBOSE_LOG } from "./flags.js";
import { getBoardsFromMesh, unshareProvokingForWood, WoodAssetsDef, } from "./wood.js";
import { BOAT_MESH, BULLET_MESH, createRudderMesh, CUBE_MESH, DBG_FABRIC, GRID_PLANE_MESH, HEX_MESH, makePlaneMesh, makeSailMesh, mkHalfEdgeQuadMesh, mkOctogonMesh, mkTimberSplinterEnd, SHIP_OFFSET, TETRA_MESH, TRI_FENCE, } from "./primatives.js";
...
const MeshTransforms = {
    ...
    rudder: mat4.translate(mat4.fromYRotation(-Math.PI * 0.5, mat4.create()), V(-5, 0, 0), mat4.create()),
    ocean: mat4.fromScaling([2, 2, 2], mat4.create()),
    ship_fangs: mat4.fromScaling([3, 3, 3], mat4.create()),
    ship_small: mat4.fromRotationTranslationScaleOrigin(quat.IDENTITY, [0, 0, 0], [6, 6, 6], [0, 0, 0], mat4.create()),
    ld51_cannon: mat4.fromRotationTranslationScale(quat.rotateX(quat.IDENTITY, Math.PI * -0.5, quat.create()), [0, 0, 0], 
    // [0.8, 0.8, 0.8], // LD51 size
    [1.2, 1.2, 1.2], mat4.create()),
    // TODO(@darzu): FOR LD53
...
const MeshModify = {
    ship_fangs: (m) => {
        ...
        const woodAssets = EM.getResource(WoodAssetsDef) ?? EM.addResource(WoodAssetsDef);
    ...
    ocean: (m) => {
        ...
        setUV(0, Math.floor(yLen / 2), vec2.clone([1, 0]), vec2.clone([0, 0]), true);
