...
import { PERF_DBG_F32S_BLAME, PERF_DBG_F32S_TEMP_BLAME, PERF_DBG_GPU_BLAME, } from "./flags.js";
...
g.cameraFollow.pitchOffset = ${target.cameraFollow.pitchOffset.toFixed(3)};
    ...
    f32sBlame: () => {
        assert(PERF_DBG_F32S_BLAME);
        const ents = [...dbgGetBlame("f32s").entries()].filter((e) => e[0] !== "Error" &&
    ...
    tempf32sBlame: () => {
        ...
        assert(PERF_DBG_F32S_TEMP_BLAME);
        const ents = [...dbgGetBlame("temp_f32s").entries()].filter((e) => e[0] !== "Error" &&
            !e[0].includes("sprig-matrix.js") &&
            !e[0].endsWith("(<anonymous>)"));
        ents.sort((a, b) => b[1] - a[1]);
        let res = ``;
    ...
    gpuBlame: () => {
        assert(PERF_DBG_GPU_BLAME);
        const ents = [...dbgGetBlame("gpu").entries()].filter((e) => e[0] !== "Error"
        // &&
        // !e[0].includes("sprig-matrix.js") &&
        // !e[0].endsWith("(<anonymous>)")
        );
