...
import { initOcean, OceanDef, oceanJfa, UVPosDef } from "./ocean.js";
import { vec2, vec3, V } from "../../sprig-matrix.js";
import { createSpawner } from "./uv-spawner.js";
...
function spawnRandomDarkStar(res, approxPosition, color) {
    const orbitalAxis = V(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5);
...
export async function initHyperspaceGame(em) {
    ...
    if (me.host) {
        // experimental ship:
        const eShip = em.new();
        em.ensureComponentOn(eShip, RenderableConstructDef, res.assets.ship_fangs.proto);
        ...
        for (let i = 0; i < NUM_ENEMY; i++) {
            let enemyUVPos = vec2.clone([Math.random(), Math.random()]);
        ...
        vec3.normalize(starPosition, starPosition);
        vec3.scale(starPosition, vec3.length(approxPosition), starPosition);
        const star1 = spawnRandomDarkStar(res, V(-1000, 2000, -1000), STAR1_COLOR
        //V(0, 0, 0)
        );
