...
import { tV, V, vec3, vec2 } from "../sprig-matrix.js";
...
EM.registerSystem([ShipDef, PositionDef, WorldFrameDef, PhysicsStateDef, LinearVelocityDef], [PartyDef, LandDef, LevelMapDef], (es, res) => {
    if (!es.length)
        return;
    const ship = es[0];
    ...
    // res.party.dir is Z
    vec2.set(res.party.dir[0] * halfLength, res.party.dir[2] * halfLength, zBasis);
    ...
    const corners = cornersFromCenter.map((v) => vec2.add(shipCenter, v));
    //console.log(corners);
    function isLand(x, z) {
        return (x < -WORLD_HEIGHT / 2 ||
            x > WORLD_HEIGHT / 2 ||
            z < -WORLD_WIDTH / 2 ||
            z > WORLD_WIDTH / 2 ||
    ...
    for (let i = 0; i < corners.length; i++) {
        ...
        for (let s = 1; s <= SAMPLES_PER_EDGE; s++) {
            ...
            const point = vec2.add(vec2.scale(corner, 1 - r, scaledTemp1), vec2.scale(neighbor, r, scaledTemp2), pointTemp);
            //console.log(point);
            if (isLand(point[0], point[1])) {
                //console.log(`touching land at face ${i}`);
                const dist = vec2.sub(neighbor, corner, pointTemp);
                const nudge = vec3.cross([0, 1, 0], vec3.set(dist[0], 0, dist[1], nudgeTemp), nudgeTemp);
                vec3.normalize(nudge, nudge);
                vec3.scale(nudge, NUDGE_DIST, nudge);
                console.log(`nudging by ${vec3Dbg(nudge)}`);
                // TODO: this should be in world space
...
//# sourceMappingURL=land-ship.js.map