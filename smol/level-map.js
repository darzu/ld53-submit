...
import { aabbCenter2, updateAABBWithPoint2_, } from "../physics/aabb.js";
import { CY } from "../render/gpu-registry.js";
import { RendererDef } from "../render/renderer-ecs.js";
import { V, vec2, vec4 } from "../sprig-matrix.js";
import { assert, assertDbg } from "../util.js";
import { vec4Dbg } from "../utils-3d.js";
import { randColor } from "../utils-game.js";
import { MapBytesSetDef, MapHelp } from "./map-loader.js";
...
function centerOfMassAndDirection(b) {
    let cx = 0;
    let cy = 0;
    let len = 0;
...
function parseAndMutateIntoMapBlobs(rgbaBytes, width, height) {
    const blobs = [];
    ...
    function parseBlob(x, y) {
        ...
        const checkColor = (x, y) => vec4.equals(color, getColor(x, y));
        parseRun(x, y);
        blob.runs.sort((a, b) => a.y - b.y);
        return blob;
        function parseRun(x, y) {
...
export function parseAndMutateIntoMapData(mapBytes, name) {
    ...
    const blobs = parseAndMutateIntoMapBlobs(buf, mapBytes.width, mapBytes.height);
    if (VERBOSE_LOG)
        for (let b of blobs)
    ...
    // }
    // extract start pos
    const startBlob = blobs.filter((b) => b.color[0] < 100 && b.color[1] < 100 && b.color[2] > 200)[0];
    ...
    towers.forEach(([pos, _]) => {
        for (let i = -towerIslandRadius; i < towerIslandRadius; i++) {
            for (let j = -towerIslandRadius; j < towerIslandRadius; j++) {
                if (i * i + j * j > towerIslandRadius * towerIslandRadius)
                    continue;
                const x = pos[0] + i;
                const y = pos[1] + j;
    ...
    const windBlobs = blobs.filter((b) => b.color[0] > 200 && b.color[1] < 150 && b.color[2] > 200);
    assert(windBlobs.length === 1, `expected 1 windBlob, found ${windBlobs.length}`);
    const windBlob = windBlobs[0];
    const [_, windDir] = centerOfMassAndDirection(windBlob);
    const endZoneBlobs = blobs.filter((b) => b.color[0] < 100 && b.color[1] > 200 && b.color[2] < 100);
    assert(endZoneBlobs.length === 1, `expected 1 end zone, found ${endZoneBlobs.length}`);
...
export async function setMap(em, name) {
    ...
    const res = await em.whenResources(MapBytesSetDef, RendererDef, ScoreDef, TextDef);
    let __start = performance.now();
    let levelMap;
    let totalPurple = 0;
...
//# sourceMappingURL=level-map.js.map