...
const MAX_TERRA_VERTS = MAX_INDICES;
const MAX_TERRA_MESHES = 500;
// TODO(@darzu): change
export const TerraVertStruct = createCyStruct({
    position: "vec3<f32>",
    normal: "vec3<f32>",
    surfaceId: "u32",
}, {
...
export const TerraUniStruct = createCyStruct({
...
const tempVertsData = [];
function computeTerraVertsData(m, startIdx, count) {
...
export function computeTerraUniData(m) {
    // TODO(@darzu): change
...
export const RenderDataTerraDef = EM.defineComponent("renderDataTerra", (r) => r);
export const terraPoolPtr = CY.createMeshPool("terraPool", {
    computeVertsData: computeTerraVertsData,
    // TODO(@darzu): per-mesh unis should maybe be optional? I don't think
    //     the terra needs them
    computeUniData: computeTerraUniData,
    vertsPtr: CY.createArray("terraVertsBuf", {
        struct: TerraVertStruct,
        init: MAX_TERRA_VERTS,
    }),
    unisPtr: CY.createArray("terraUni", {
        struct: TerraUniStruct,
        init: MAX_TERRA_MESHES,
    }),
    triIndsPtr: CY.createIdxBuf("terraTriIndsBuf", { init: MAX_TERRA_VERTS * 3 }),
    lineIndsPtr: CY.createIdxBuf("terraLineIndsBuf", {
        init: MAX_TERRA_VERTS * 2,
    }),
    // TODO(@darzu): this dataDef is v weird
    dataDef: RenderDataTerraDef,
});
export const TerraCutTexPtr = CY.createTexture("terraCut", {
    size: [1024, 512],
    // TODO(@darzu): we want the smaller format
    format: "r32float",
    // format: "r8unorm",
});
export const renderTerraPipe = CY.createRenderPipeline("terraRender", {
    globals: [
        sceneBufPtr,
  ...
  ${shaderSet["xp-terra"].code}
  `,
});
onInit((em) => {
    em.registerSystem([RenderableDef, RenderDataTerraDef, RendererWorldFrameDef], [RendererDef], (objs, res) => {
        const pool = res.renderer.renderer.getCyResource(terraPoolPtr);
        for (let o of objs) {
            // TODO(@darzu): do we need all this for terra?
            // color / tint
            if (ColorDef.isOn(o)) {
                vec3.copy(o.renderDataTerra.tint, o.color);
            }
            if (TintsDef.isOn(o)) {
                applyTints(o.tints, o.renderDataTerra.tint);
            }
            // id
            // o.renderDataTerra.id = o.renderable.meshHandle.mId;
            // transform
            mat4.copy(o.renderDataTerra.transform, o.rendererWorldFrame.transform);
            pool.updateUniform(o.renderable.meshHandle, o.renderDataTerra);
        }
    }, "updateTerraRenderData");
    em.requireSystem("updateTerraRenderData");
    em.addConstraint(["updateTerraRenderData", "after", "renderList"]);
    em.addConstraint(["updateTerraRenderData", "before", "stepRenderer"]);
});
//# sourceMappingURL=std-terrain.js.map