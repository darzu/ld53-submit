...
async function initDragBox() {
    ...
    EM.registerSystem(null, [MouseDragDef, CameraComputedDef, WidgetLayerDef], (_, { mousedrag, cameraComputed, widgets }) => {
        // update dragbox
        if (widgets.cursor || mousedrag.isDragEnd) {
            ...
            // place dragbox
            const min = screenPosToWorldPos(tempVec3(), mousedrag.dragMin, cameraComputed);
            min[1] = 0;
            const max = screenPosToWorldPos(tempVec3(), mousedrag.dragMax, cameraComputed);
            max[1] = 1;
...
async function initWidgets({ assets }) {
    ...
    EM.registerSystem(null, [
        ...
        if (mousedrag.isDragging) {
            const start = screenPosToWorldPos(tempVec3(), mousedrag.dragLastEnd, cameraComputed);
            start[1] = 0;
            const end = screenPosToWorldPos(tempVec3(), mousedrag.dragEnd, cameraComputed);
            end[1] = 0;
    ...
    EM.registerSystem([WidgetDef, ColorDef], [WidgetLayerDef], (ws, { widgets }) => {
