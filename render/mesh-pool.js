...
import { createCyArray, createCyIdxBuf, } from "./data-webgpu.js";
...
function logMeshPoolStats(pool) {
...
device, resources, ptr) {
    ...
    const pushNewBuffSet = () => {
        assert(sets.length + 1 <= ptr.maxSets, `Too many mesh-pool BuffSet! max: ${ptr.maxSets}`);
    ...
    function addMesh(m, reserved) {
        ...
        // console.dir(m);
        assert(m.colors.length === faceNum, `${m.dbgName}: Inconsistent face num ${faceNum} vs color num ${m.colors.length}`);
        assert(m.surfaceIds.length === faceNum, `${m.dbgName}: Inconsistent face num ${faceNum} vs surface IDs num ${m.surfaceIds.length}`);
        ...
        const doesFit = pool.sets[currSetIdx].numVerts + vertNum <= ptr.setMaxVerts &&
        ...
        // submit data to GPU
        if (m.quad.length)
            updateMeshQuads(handle, m);
        if (m.tri.length)
            updateMeshTriangles(handle, m);
        if (m.pos.length)
            updateMeshVertices(handle, m);
    ...
    function updateMeshInstance(m, proto) {
    ...
    function updateMeshSize(handle, newMesh) {
        const m = newMesh;
        ...
        assert((m.lines?.length ?? 0) <= handle.reserved.maxLineNum, `Too many lines! ${m.lines?.length} vs ${handle.reserved.maxLineNum}`);
        // TODO(@darzu): what to do about this requirement...
        assert(!m.quad.length || m.tri.length % 2 === 0, `tri.length not even for ${m.dbgName}`);
    ...
    function updateUniform(m, d) {
        ...
        if (PERF_DBG_GPU)
            _stats._accumUniDataQueued += pool.unis.struct.size;
    }
