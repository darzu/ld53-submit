...
import { CannonLocalDef, createCannonNow, } from "../games/cannon.js";
...
export async function createShip(em) {
    ...
    em.ensureComponentOn(ent, RenderableConstructDef, homeShip.timberMesh
    ...
    // create cannons
    const cannonR = createCannonNow(res, V(-8, 4.7, -7), Math.PI * 0.5, cannonDefaultPitch, ent.id);
    vec3.copy(cannonR.color, ENDESGA16.darkGray);
    ent.ld52ship.cannonR = createRef(cannonR);
    const cannonL = createCannonNow(res, V(8, 4.7, -7), Math.PI * 1.5, cannonDefaultPitch, ent.id);
...
async function createRudder(em) {
    ...
    em.ensureComponentOn(ent, RenderableConstructDef, res.assets.rudderPrim.proto);
