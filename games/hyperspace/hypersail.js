...
import { DEFAULT_SAIL_COLOR, getSailMeshArea, sailForceAndSignedArea, } from "./ribsail.js";
...
export const { HypMastPropsDef, HypMastLocalDef, createHypMastNow } = defineNetEntityHelper(EM, {
    name: "hypMast",
    defaultProps: (shipId) => ({
        shipId: shipId ?? 0,
    }),
    ...
    build: (mast, res) => {
        ...
        em.ensureComponentOn(interactBox, PhysicsParentDef, mast.hypMastProps.shipId);
        ...
        constructNetTurret(mast, 0, 0, interactBox, Math.PI, -Math.PI / 8, -1, V(0, 20, 50));
...
onInit((em) => {
    em.registerSystem([HypMastPropsDef, HypMastLocalDef, TurretDef, BoomPitchesDef], [InputsDef, RendererDef], (masts, res) => {
    ...
    em.registerSystem([PlayerShipPropsDef, UVShipDef, WorldFrameDef, AuthorityDef], [MeDef, GameStateDef], (es, res) => {
        ...
        for (let ship of es) {
            ...
            for (let sail of sails) {
                ...
                const [force, area] = sailForceAndSignedArea(sail, star.world.position);
                const accel = vec3.dot(shipDirection, force);
                const realArea = Math.abs(getSailMeshArea(sail.renderable.meshHandle.mesh.pos));
                // console.log(
                //   `Color lerp is ${
                //     accel / realArea
...
//# sourceMappingURL=hypersail.js.map