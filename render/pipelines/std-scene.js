...
export function computeVertsData(m, startIdx, count) {
    ...
    assertDbg(!m.uvs || m.uvs.length === m.pos.length, `uvs.length != pos.length for ${m.dbgName}`);
    ...
    for (let vi = startIdx; vi < startIdx + count; vi++) {
        ...
        if (m.uvs)
            tempVertsData[dIdx].uv = m.uvs[vi];
...
export const canvasFormat = navigator.gpu?.getPreferredCanvasFormat() ?? "bgra8unorm";
