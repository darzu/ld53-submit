import { vec2, vec3, quat, mat4, V } from "./sprig-matrix.js";
import { avg } from "./math.js";
import { createAABB, getAABBFromPositions } from "./physics/aabb.js";
import { tempVec2, tempVec3 } from "./temp-pool.js";
import { assertDbg, resizeArray } from "./util.js";
...
export function computeTriangleNormal(p1, p2, p3, out) {
...
export function randNormalPosVec3(out) {
    if (!out)
        out = vec3.create();
...
export function vec3Dbg2(v, precision = 2) {
    return `V(${v[0].toFixed(precision)},${v[1].toFixed(precision)},${v[2].toFixed(precision)})`;
}
export function vec4Dbg(v) {
    return `[${v[0].toFixed(2)},${v[1].toFixed(2)},${v[2].toFixed(2)},${v[3].toFixed(2)}]`;
}
export function vec4Dbg2(v, precision = 2) {
    return `V(${v[0].toFixed(precision)},${v[1].toFixed(precision)},${v[2].toFixed(precision)},${v[3].toFixed(precision)})`;
}
...
export function orthonormalize(forward, upish, outRight) {
...
const _tempWorldCorners = screenCorners.map((_) => V(0, 0, 0));
export function getFrustumWorldCorners(invViewProj, out) {
...
const _tempViewCorners = [];
const _tempViewAABB = createAABB();
export function frustumFromBounds(worldCorners, eyePos, outFrust) {
    // view matrix
    ...
    worldCorners.forEach((p, i) => vec3.transformMat4(p, viewTmp, _tempViewCorners[i]));
    ...
    // left/right
    _tempViewAABB.min[0], _tempViewAABB.max[0], 
    // bottom/top
    _tempViewAABB.min[1], _tempViewAABB.max[1], 
    // near/far
    -1.0, // TODO(@darzu): HACK?
    // -_tempViewAABB.max[2],
    -_tempViewAABB.min[2]);
...
export function angleBetweenPosXZ(pos, rot, fwd, // NOTE: fwd needs to be normalized and with 0 y component
target) {
    const toward = vec3.normalize([target[0] - pos[0], 0, target[2] - pos[2]], __angleBetweenXZTmp0);
...
export function angleBetweenXZ(norm1, norm2) {
