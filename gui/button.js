...
f 7// 8// 3// 2//
`;
export const ButtonDef = EM.defineComponent("button", (key, data, colors) => ({
    key,
...
export const ButtonsStateDef = EM.defineComponent("buttonsState", (gmesh) => ({
...
async function initButtonGUI(res) {
    // init ButtonsStateDef
    {
        const btnMesh_ = importObj(BTN_OBJ);
        assert(typeof btnMesh_ !== "string" && btnMesh_.length === 1, `btn mesh failed import: ${btnMesh_}`);
    ...
    EM.registerSystem([ButtonDef], [PhysicsResultsDef, ButtonsStateDef, InputsDef, UICursorDef], (es, res) => {
        // reset by-key state
        ...
        for (let btn of es) {
            ...
            const isHover = (colW ?? []).some((oId) => oId === res.uiCursor.cursor.id);
            const wasHover = res.buttonsState.hover[btn.id];
            const wasDown = res.buttonsState.down[btn.id];
            // hover
            if (isHover)
                res.buttonsState.hover[btn.id] = true;
            else
                res.buttonsState.hover[btn.id] = false;
            // down
            // TODO(@darzu): drag from outside?
            if (isHover && res.inputs.ldown)
                res.buttonsState.down[btn.id] = true;
            else
                res.buttonsState.down[btn.id] = false;
            // click
            if (isHover && wasDown && !res.inputs.ldown) {
                ...
                res.buttonsState.click[btn.id] = false;
        }
    }, "buttonStateUpdate");
    EM.requireGameplaySystem("buttonStateUpdate");
    EM.registerSystem([ButtonDef, ColorDef], [ButtonsStateDef], (es, res) => {
        for (let btn of es) {
            ...
            if (isHover)
                vec3.copy(btn.color, colors.hover);
            if (isDown)
                vec3.copy(btn.color, colors.down);
