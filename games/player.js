...
// player controller component and system
import { vec3, quat, V } from "../sprig-matrix.js";
    ...
    em.registerSystem([PlayerPropsDef], [MeDef, AssetsDef], (players, res) => {
        for (let e of players) {
            ...
            if (!RotationDef.isOn(e))
                em.addComponent(e.id, RotationDef, quat.rotateY(quat.IDENTITY, Math.PI, quat.create()));
            if (!LinearVelocityDef.isOn(e))
                em.addComponent(e.id, LinearVelocityDef);
            // console.log("making player!");
            if (!ColorDef.isOn(e))
                em.addComponent(e.id, ColorDef, V(0, 0.2, 0));
    ...
    em.registerSystem([
        ...
        for (let p of players) {
            ...
            if (cheat && inputs.rclick) {
                ...
                for (let xi = 0; xi <= SPREAD; xi++) {
                    for (let yi = 0; yi <= SPREAD; yi++) {
                        ...
                        const position = vec3.add(p.world.position, V(x, y, 0), vec3.create());
            ...
            function playerShootRay(r) {
                ...
                const color = doesHit ? V(0, 1, 0) : V(1, 0, 0);
