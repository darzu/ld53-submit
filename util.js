...
//  numbers.
let __logBatch = "";
export function dbgLogLineBatch(msg) {
    __logBatch += msg + "\n";
}
export function dbgLogNextBatch() {
    if (__logBatch)
        console.log(__logBatch);
    __logBatch = "";
}
...
export function boxInBox(boundW, boundH, childAR) {
...
export function resizeArray(arr, len, make // TS HACK: we have to use `T2 extends T` here otherwise TS would allow T to infer as that intersection of arr and make
) {
