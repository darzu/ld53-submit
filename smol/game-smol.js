...
import { mat3, quat, V, vec3 } from "../sprig-matrix.js";
...
import { GrassCutTexPtr, grassPoolPtr } from "./std-grass.js";
...
import { LevelMapDef, LandMapTexPtr } from "./level-map.js";
...
export async function initSmol(em, hosting) {
    const dbgGrid = [
        //
    ...
    if (!"true")
        for (let r = 0; r < 2; r++)
            for (let i = 0; i < 8; i++) {
                ...
                em.ensureComponentOn(bigCube, RenderableConstructDef, even ? res.assets.ball.proto : res.assets.cube.proto);
                em.ensureComponentOn(bigCube, ScaleDef, V(50, 50, 50));
                em.ensureComponentOn(bigCube, RotationDef, quat.fromEuler(Math.random() * Math.PI, Math.random() * Math.PI, Math.random() * Math.PI, quat.create()));
                em.ensureComponentOn(bigCube, AngularVelocityDef, vec3.scale(randNormalVec3(vec3.tmp()), 0.0005, vec3.create()));
                em.ensureComponentOn(bigCube, PositionDef, V(
                // i2 * 30 + even * 100 - 100 - r * 50,
                jitter(WORLD_HEIGHT * 0.5), r * 100, 
                // i2 * 30 - even * 100 - 100 + r * 50
    ...
    em.ensureComponentOn(sky, RenderableConstructDef, skyMesh, undefined, undefined, SKY_MASK);
    ...
    const gr = em.new();
    em.ensureComponentOn(gr, RenderableConstructDef, grMesh, undefined, 
    // false,
    undefined, undefined, grassPoolPtr
    // true
    );
    ...
    // update grass
    EM.registerSystem([CameraFollowDef, WorldFrameDef], [], (es, res) => {
        const player = es[0];
    ...
    assert(WORLD_WIDTH === grassCutTex.size[0] && WORLD_HEIGHT === grassCutTex.size[1]);
    ...
    EM.registerSystem([ShipDef, PositionDef, WorldFrameDef, PhysicsStateDef], [PartyDef, LevelMapDef, ScoreDef], (es, res) => {
        if (!es.length)
            return;
        const ship = es[0];
        ...
        if (hasUpdate) {
            ...
            grassCutTex.queueUpdate(windowData, minWinXi, minWinYi, innerWinW, innerWinH);
        }
        // console.dir(data);
        // rasterizeTri
    }, "cutGrassUnderShip");
    ...
    // Spawn towers
    {
        const tower3DPoses = level.levelMap.towers.map((tPos) => level2DtoWorld3D(tPos[0], 20, // TODO(@darzu): lookup from heightmap?
        vec3.create()));
    ...
    EM.ensureComponentOn(worldGizmo, PositionDef, V(-WORLD_HEIGHT / 2, 0, -WORLD_WIDTH / 2));
    EM.ensureComponentOn(worldGizmo, ScaleDef, V(100, 100, 100));
    EM.ensureComponentOn(worldGizmo, RenderableConstructDef, res.assets.gizmo.proto);
...
//   }
// }
//# sourceMappingURL=game-smol.js.map