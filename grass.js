...
export function createGrassTile(opts) {
    ...
    // TODO(@darzu): use this?
    const aabbMin = V(-spacing, 0, -spacing);
    const aabbMax = V(size + spacing, bladeH * 2, size + spacing);
...
// }
export async function createGrassTileset(opts) {
    ...
    for (let xi = 0; xi < tilesPerSide; xi++) {
        for (let zi = 0; zi < tilesPerSide; zi++) {
            ...
            EM.ensureComponentOn(tile, RenderableConstructDef, tileProto, 
            // false,
            undefined, undefined, undefined, grassPoolPtr);
            EM.ensureComponentOn(tile, ColorDef, randColor());
            // mat4.translate(tile.transform, tile.transform, [x, 0, z]);
            // builder.updateUniform(tile);
            tileProms.push(EM.whenEntityHas(tile, PositionDef, RenderableDef, RenderDataGrassDef));
        }
    }
