...
export function createWaves() {
    ...
    const speedFromFreq = (w) => Math.sqrt(GRAV / w);
    const speedFromLen = (l) => Math.sqrt((GRAV * l) / (2 * Math.PI));
    const p1 = [
    ...
    for (let p of p1)
        res.push(mkGerstnerFromDirLenSteep(p));
    for (let p of p2)
        res.push(mkGerstnerFromDirLenAmpSpeed(p));
    return res;
    function mkGerstnerFromDirLenSteep(params) {
    ...
    function mkGerstnerFromDirLenAmpSpeed(params) {
        const D = vec2.fromRadians(params.dirRad, V(0, 0));
        const w = (2 * Math.PI) / params.len;
        const A = params.amp;
        const Q = 0;
...
function mkGerstnerWaveTS(params) {
    return {
        D: params.D ?? V(1, 0),
        Q: params.Q ?? 0,
        A: params.A ?? 1,
...
export function compute_gerstner(outDisp, outNorm, waves, uv, t // ms
) {
    vec3.zero(outDisp);
    vec3.zero(outNorm);
...
function catlike_gerstner(steepness, wavelength, dir, t, uv, p) {
...
// test_gerstner();
//# sourceMappingURL=gerstner.js.map