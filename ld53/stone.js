...
export const towerPool = createEntityPool({
    ...
    create: async () => {
        ...
        const tower = EM.new();
        const cannon = EM.new();
        EM.ensureComponentOn(cannon, RenderableConstructDef, res.assets.ld53_cannon.proto);
...
export const flyingBrickPool = createEntityPool({
    ...
    create: async () => {
        ...
        EM.ensureComponentOn(brick, ScaleDef, V(approxBrickWidth / 2, approxBrickHeight / 2, brickDepth / 2));
...
EM.registerSystem([FlyingBrickDef, DeadDef], [], (es, _) => es.forEach((e) => {
    if (!e.dead.processed) {
...
const MAX_RANGE = 300;
EM.registerSystem([StoneTowerDef, WorldFrameDef], [TimeDef, PartyDef], (es, res) => {
...
EM.registerSystem([StoneTowerDef, RenderableDef, WorldFrameDef], [PhysicsResultsDef, RendererDef], (es, res) => {
