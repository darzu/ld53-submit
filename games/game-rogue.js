...
import { vec3, quat, mat4, V } from "../sprig-matrix.js";
...
import { cloneMesh, getAABBFromMesh, scaleMesh3, transformMesh, } from "../render/mesh.js";
...
export async function initRogueGame(em, hosting) {
    ...
    // TIMBER
    const timber = em.new();
    const { timberState, timberMesh, ribCount, ribSpace, ribWidth, ceilHeight, floorHeight, floorLength, floorWidth, } = createHomeShip();
    ...
    for (let i = 0; i < 2; i++) {
        ...
        em.ensureComponentOn(cannon, PositionDef, V(-7.5, realFloorHeight + 2, -4 * isLeft));
    ...
    if (DBG_PLAYER)
        EM.requireGameplaySystem("ld51Ghost");
...
//# sourceMappingURL=game-rogue.js.map