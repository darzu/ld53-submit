...
export const { CannonPropsDef, CannonLocalDef, createCannon, createCannonNow } = defineNetEntityHelper(EM, {
    ...
    build: (e, res) => {
        ...
        constructNetTurret(e, props.yaw, props.pitch, res.assets.ld51_cannon.aabb, 0, undefined, undefined, undefined, true, 1.0, Math.PI / 4, "W/S: pitch, A/D: turn, left click: fire, E: drop cannon");
        em.ensureComponentOn(e, ColorDef, V(0, 0, 0));
        em.ensureComponentOn(e, RenderableConstructDef, res.assets.ld51_cannon.mesh);
...
export function registerCannonSystems(em) {
    ...
    const raiseFireCannon = eventWizard("fire-cannon", [[PlayerDef], [CannonLocalDef, WorldFrameDef]], ([player, cannon]) => {
        ...
        if (player.id === EM.getResource(LocalPlayerDef)?.playerId) {
            ...
            const v = 0.18;
            const g = 6.0 * 0.00001;
            const b = fireBullet(EM, 2, firePos, fireDir, v, 0.02, g, 
            // 2.0,
            20.0, [0, 0, -1]);
        }
