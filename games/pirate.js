...
import { WoodHealthDef, WoodStateDef, registerDestroyPirateHandler, createEmptyMesh, createTimberBuilder, getBoardsFromMesh, verifyUnsharedProvokingForWood, reserveSplinterSpace, createWoodHealth, resetWoodHealth, resetWoodState, } from "../wood.js";
...
export const PiratePlatformDef = EM.defineComponent("piratePlatform", (cannon, timber) => {
    return {
        ...
        for (let p of ps) {
            ...
            if (doFire) {
                ...
                if (WorldFrameDef.isOn(c)) {
                    ...
                    fireBullet(em, 2, c.world.position, c.world.rotation, 0.05, 0.02, 3 * 0.00001, ballHealth, [0, 0, -1]);
                }
            }
        }
...
const piratePool = createEntityPool({
    ...
    create: async () => {
        ...
        transformMesh(groundMesh, mat4.fromRotationTranslationScale(quat.IDENTITY, [0, -1, 0], [4, 1, 4]));
...
export function destroyPirateShip(id) {
    const pirate = EM.findEntity(id, [
        PiratePlatformDef,
        PositionDef,
        RotationDef,
    ]);
    if (pirate && !DeadDef.isOn(pirate))
        piratePool.despawn(pirate);
}
