...
function wgslTypeToDummyVal(wgsl) {
    ...
    function _wgslTypeToDummyVal(wgsl) {
        ...
        if (wgsl === "vec3<f32>")
            return randVec3();
        const randVec4 = () => vec4.fromValues(Math.random(), Math.random(), Math.random(), Math.random());
        if (wgsl === "vec4<f32>")
            return randVec4();
        if (wgsl === "u32")
            return Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
        const randAngle = () => Math.random() * 2 * Math.PI;
        const randQuat = () => quat.fromEuler(randAngle(), randAngle(), randAngle(), quat.create());
        if (wgsl === "mat4x4<f32>")
