...
import { mat4, vec3, quat, tV } from "../sprig-matrix.js";
import { clamp, jitter } from "../math.js";
import { createAABB, updateAABBWithPoint, aabbCenter, } from "../physics/aabb.js";
import { ColliderDef } from "../physics/collider.js";
import { PositionDef, RotationDef, PhysicsParentDef, ScaleDef, } from "../physics/transform.js";
import { cloneMesh, transformMesh, getAABBFromMesh, scaleMesh, } from "../render/mesh.js";
...
import { WoodHealthDef, WoodStateDef, createEmptyMesh, createTimberBuilder, getBoardsFromMesh, verifyUnsharedProvokingForWood, reserveSplinterSpace, createWoodHealth, resetWoodHealth, resetWoodState, } from "../wood.js";
import { PartyDef } from "./party.js";
import { angleBetweenPosXZ, angleBetweenXZ, } from "../utils-3d.js";
import { createRibSailNow } from "./hyperspace/ribsail.js";
...
export const TowerPlatformDef = EM.defineComponent("towerPlatform", (cannon, timber, sail) => {
    return {
        cannon: createRef(cannon),
        timber: createRef(timber),
        sail: createRef(sail),
        lastFire: 0,
...
export async function startTowers(towerPositions) {
    const em = EM;
    // TODO(@darzu): HACK!
...
EM.registerSystem([TowerPlatformDef, PositionDef, RotationDef], [TimeDef, PartyDef, WindDef], (es, res) => {
    const target = res.party.pos;
    if (!target)
        return; // TODO(@darzu): fold up sail!
    const fwd = tV(0, 0, -1);
    const behind = tV(0, 0, 1);
    for (let tower of es) {
        const angleToParty = angleBetweenPosXZ(tower.position, tower.rotation, fwd, res.party.pos);
        // turn the tower
        ...
        quat.rotateY(quat.IDENTITY, sailAngle, tower.towerPlatform.sail().rotation);
...
const towerPool = createEntityPool({
    ...
    create: async () => {
        ...
        transformMesh(groundMesh, mat4.fromRotationTranslationScale(quat.IDENTITY, [0, -6, 0], [8, 6, 8]));
...
EM.registerSystem([TowerPlatformDef, PositionDef, RotationDef], [TimeDef, PartyDef], (es, res) => {
    ...
    for (let tower of es) {
        idx++;
        // fire cannons
        const nextFire = tower.towerPlatform.lastFire + towerFireSpeed + idx * 150;
        ...
        if (doFire) {
            ...
            fireBullet(EM, 2, cannon.world.position, cannon.world.rotation, speed, rotSpeed, gravity, ballHealth, [0, 0, -1]);
        }
    }
}, "updateTowerAttack");
