...
export function registerControllableSystems(em) {
    ...
    em.registerSystem([
        ...
        RotationDef,
        WorldFrameDef,
    ], [InputsDef, MeDef, CanvasDef, TimeDef], (controllables, res) => {
        for (let c of controllables) {
            if (c.authority.pid !== res.me.pid)
                continue;
    ...
    em.registerSystem([ControllableDef, CameraFollowDef, AuthorityDef], [InputsDef, MeDef, CanvasDef], (controllables, res) => {
        for (let c of controllables) {
            if (c.authority.pid !== res.me.pid)
                continue;
            if (c.controllable.requiresPointerLock &&
