...
import { clamp, jitter, max } from "../math.js";
...
import { createFlatQuadMesh, makeDome, resetFlatQuadMesh, } from "../primatives.js";
...
import { cannonDefaultPitch, createShip } from "../smol/ship.js";
...
import { resetWoodHealth, resetWoodState, WoodStateDef, } from "../wood.js";
...
export async function initLD53(em, hosting) {
    const dbgGrid = [
        //
    ...
    if (!"true") {
        ...
        for (let u = 0.4; u <= 0.6; u += 0.02) {
            for (let v = 0.4; v <= 0.6; v += 0.02) {
                ...
                em.ensureComponentOn(bouy, UVPosDef, V(u + jitter(0.01), v + jitter(0.01)));
        ...
        em.registerSystem([bouyDef, PositionDef, UVPosDef], [OceanDef], (es, res) => {
    ...
    score.onLevelEnd.push(async () => {
        ...
        em.whenEntityHas(ship, RenderableDef, WoodStateDef).then((ship) => res.renderer.renderer.stdPool.updateMeshQuads(ship.renderable.meshHandle, ship.woodState.mesh, 0, ship.woodState.mesh.quad.length));
        console.log("resetting dock position");
        const endZonePos = level2DtoWorld3D(level.levelMap.endZonePos, 5, vec3.tmp());
        vec3.copy(dock.position, endZonePos);
        resetWoodHealth(dock.woodHealth);
        resetWoodState(dock.woodState);
        em.whenEntityHas(dock, RenderableDef, WoodStateDef).then((dock) => res.renderer.renderer.stdPool.updateMeshQuads(dock.renderable.meshHandle, dock.woodState.mesh, 0, dock.woodState.mesh.quad.length));
        const towers = EM.filterEntities([StoneTowerDef]);
        for (let tower of towers) {
            towerPool.despawn(tower);
        }
        // spawn towers
        const tower3dPosesAndDirs = level.levelMap.towers.map(([tPos, tDir]) => [
    ...
    EM.registerSystem([], [InputsDef, PartyDef], (_, res) => {
