...
import { copyLine, createLine, emptyLine, getLineEnd, getLineMid, lineSphereIntersections, transformLine, } from "./physics/broadphase.js";
...
function createSplinterEnd(seg, boardMesh, top, W, D) {
    ...
    EM.ensureComponentOn(splinter, ColorDef, V(Math.random(), Math.random(), Math.random()));
    EM.ensureComponentOn(splinter, PositionDef);
    EM.ensureComponentOn(splinter, RotationDef);
...
export function setSideQuadIdxs(loop1Vi, loop2Vi, q0, q1, q2, q3) {
...
export function setEndQuadIdxs(loopVi, q, facingDown) {
...
export function getBoardsFromMesh(m) {
    ...
    function createBoard(startQi) {
        ...
        return undefined;
        function addBoardSegment(lastLoop, // [VI, VI, VI, VI],
        isFirstLoop = false) {
            ...
            const nextLoop = vec4.clone(nextLoop_);
            ...
            if (segQis.length === 5) {
                ...
                if (endQuads.length === 1) {
                    ...
                    const sideQuads = vec4.clone(segQis.filter((qi) => qi !== endQuad));
                    seg = {
                        localAABB: aabb,
                        midLine: mid,
                        areaNorms,
                ...
                seg = {
                    ...
                    vertLastLoopIdxs: lastLoop,
                    vertNextLoopIdxs: nextLoop,
                    quadSideIdxs: vec4.clone(segQis),
                };
            }
            // continue
